...
class PlacaControleMotor:
    ...
    def set_kp_freio(self, valor):
        """Define a constante proporcional (Kp) do freio travado.
    ...
    def set_kd_freio(self, valor):
        """Define a constante derivativa (Kd) do freio travado.
    ...
    def set_delta_freio(self, valor):
        """Define o DELTA (zona morta) do freio travado.

        Args:
            valor: Valor inteiro do delta (0-65535)
        ...
        for i in range(3):
            ...
            if self.DEBUG:
                print(f'Tentativa {i + 1}/3 falhou')
            time.sleep(0.005)
