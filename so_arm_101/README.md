...
Use `ros2 launch bringup workstation.launch.py` para o RViz do robô físico.
Os launches de Gazebo, modelo offline, demos e RViz isolado foram removidos.
...
Para mover o braço manualmente mantendo a leitura das posições, sustente-o e
desabilite o torque em runtime:
...
O modo manual ignora comandos enviados aos servos. Para voltar ao controle
normal sem reiniciar, chame o mesmo serviço com `"{data: true}"`.
...
No notebook, abra a workstation com MotionPlanning:
...
Se o detector, a transformação para `arm_base_link` ou o ID solicitado não
