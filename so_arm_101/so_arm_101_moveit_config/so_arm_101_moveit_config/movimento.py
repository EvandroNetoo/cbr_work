...
class ExecutorDoMoveIt:
    ...
    def __init__(
        ...
        *,
        ...
        self._trava_do_objetivo = threading.Lock()
    ...
    def _aguardar_futuro(self, futuro, timeout_sec: float | None = None):
        """Wait without recursively spinning a node owned by another executor."""
        if self._possui_no:
            rclpy.spin_until_future_complete(
                self.no, futuro, timeout_sec=timeout_sec
            )
            if not futuro.done():
        ...
        if futuro.exception() is not None:
            raise futuro.exception()
        return futuro.result()

    def cancelar_objetivo_ativo(self) -> None:
        with self._trava_do_objetivo:
            objetivo = self._objetivo_ativo
        if objetivo is not None:
            objetivo.cancel_goal_async()

    def _definir_objetivo_ativo(self, objetivo) -> None:
        with self._trava_do_objetivo:
            self._objetivo_ativo = objetivo
