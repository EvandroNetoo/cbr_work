...
except ModuleNotFoundError:  # Permite importar o módulo em testes sem hardware.
    SMBus = None
...
class VL53L0X:  # noqa
    ...
    def __init__(
        ...
        *,
        ...
        ranging_timeout_ms=TIMEOUT_RANGING_MS,
    ):
        if porta_mux is None or porta_mux > 7 or porta_mux < 0:
            raise ValueError('Canal inválido (deve ser 0 a 7)')
        if not 0x08 <= int(mux_address) <= 0x77:
            raise ValueError('Endereço do multiplexador I2C inválido')
        if int(ranging_timeout_ms) <= 0:
            raise ValueError('Timeout de leitura deve ser positivo')

        self.porta_mux = porta_mux
        self.I2C_BUS = int(i2c_bus)
        self.MUX_ADDR = int(mux_address)
        self.TIMEOUT_RANGING_MS = int(ranging_timeout_ms)
    ...
    def close(self):
        """Interrompe o sensor e fecha o barramento apenas quando ele é próprio."""
        if self._closed:
            return
        self._closed = True
        try:
