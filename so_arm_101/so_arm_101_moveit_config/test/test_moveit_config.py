...
@pytest.mark.parametrize(
    ...
    (
        (
            'deposit_cube_left',
            [1.6580627894, 0.8028514559, -0.9424777961,
             -1.5533430343, 1.6406094969],
        ),
        (
            'safe_cube_left',
            [1.6755160819, 0.7679448709, -0.5759586532,
             -1.7802358370, 1.5707963268],
        ),
        (
            'pick_cube_left',
            [1.6755160819, 0.9599310886, -1.0995574288,
             -1.4137166941, 1.5707963268],
        ),
    ),
)
def test_left_cargo_named_states_use_requested_joint_values(state_name,
                                                             expected):
    root = ET.parse(os.path.join(CONFIG_DIR, 'so_arm_101.srdf')).getroot()
